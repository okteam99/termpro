...
const builtAtArg = process.argv.find((argument) =>
  argument.startsWith('--built-at='),
);
const screenshotsDirArg = process.argv.find((argument) =>
  argument.startsWith('--screenshots-dir='),
);
const screenshotsDir = screenshotsDirArg
  ? path.resolve(
      process.cwd(),
      screenshotsDirArg.slice('--screenshots-dir='.length),
    )
  : path.join(
      root,
      'docs',
      'features',
      'OKWORK-F260807022801-Profile-Password-Vault',
      'screenshots',
    );
...
let buildStartedAt = builtAtArg
  ? Number(builtAtArg.slice('--built-at='.length))
  : 0;
...
if (!skipBuild) {
  ...
  check(
    includesAll(
      mainBundle,
      'browserGuestPreload.js',
      'passwordTrustedPreload.js',
      'passwordTrusted',
    ),
  ...
  check(
    mainBundle.includes('passwordVaultGuest:lookup') &&
      mainBundle.includes('passwordVaultTrusted:reveal'),
    'main bundle owns separate guest and trusted IPC boundaries',
  );
  check(
    includesAll(
      guestPreload,
      'passwordVaultGuest:lookup',
      'passwordVaultGuest:candidate',
      'passwordVaultGuest:result',
  ...
  check(
    includesAll(
      trustedPreload,
      'passwordVaultTrusted:context',
      'passwordVaultTrusted:reveal',
      'passwordVaultTrusted:copy',
  ...
  check(
    includesAll(
      ordinaryPreload,
      'passwordVault:capabilities',
      'passwordVault:listMetadata',
      'passwordVault:openTrusted',
    ) &&
  ...
  // AC-6 and AC-8: both renderer routes ship, while browser chrome exposes the
  // selected storage location without restoring the removed description bubble.
  check(
  ...
  check(
    renderer.includes('Password storage'),
    'browser chrome artifact includes the storage location label',
  );
  check(
  ...
  function startLoginFixture() {
    const server = http.createServer((request, response) => {
      ...
      if (url.pathname === '/signed-in') {
        response.end(
          '<!doctype html><title>Signed in</title><main id="signed-in">Signed in</main>',
        );
        return;
      }
  ...
  function assertJourney(condition, description) {
    if (!condition)
      throw new Error(`T-012 journey assertion failed: ${description}`);
    console.log(`PASS ${description}`);
  }
  ...
  async function run_password_vault_browser_journeys() {
    ...
    const userDataDir = fs.mkdtempSync(
      path.join(os.tmpdir(), 'okwork-password-vault-e2e-'),
    );
    let fixture;
    let app;
    ...
    try {
      ...
      await eventually(
        () =>
          mainWindow
            .locator('button[title="Show browser"]')
      ...
      await browserWindow
        .locator('.browser-panel__address-input')
        .waitFor({ state: 'visible' });
      const browserChromeText = await browserWindow
        .locator('.browser-panel')
        .innerText();
      assertJourney(
        browserChromeText.includes('Password storage: This device'),
        'OkBrowser chrome shows the current password storage location as ordinary text',
      );
      assertJourney(
        (await browserWindow
          .locator('.password-status__disclosure')
          .count()) === 0 && !browserChromeText.includes('AUTHORITY'),
        'OkBrowser chrome does not restore the removed description bubble or AUTHORITY marker',
      );
      ...
      async function guestEvaluate(code) {
        return eventually(
          () =>
            webview.evaluate(
              (element, script) => element.executeJavaScript(script),
              code,
            ),
      ...
      await eventually(async () => {
        const ready = await guestEvaluate(
      ...
      await eventually(async () => {
        const signedIn = await guestEvaluate(
          "Boolean(document.querySelector('#signed-in'))",
        );
      ...
      await eventually(async () => {
        ...
        if (!text.includes('New password saved automatically'))
          throw new Error('saved status absent');
      ...
      assertJourney(
        (await browserWindow.locator('.browser-panel').innerText()).includes(
          'Password filled from',
        ),
      ...
      await eventually(async () => {
        ...
        if (!preserved)
          throw new Error('silent fill overwrote a non-empty field');
      ...
      await mainWindow
        .getByRole('menuitem', { name: 'Saved Passwords' })
        .click();
      await mainWindow
        .getByRole('heading', { name: 'Saved Passwords' })
        .waitFor({ state: 'visible' });
      const savedPasswordsText = await mainWindow
        .locator('.saved-passwords')
        .innerText();
      assertJourney(
        savedPasswordsText.includes(fixture.origin) &&
          savedPasswordsText.includes('alice'),
        ...
        const result = await trustedWindow
          ...
          .evaluate(
            (element, expected) => ({
              ...
              error:
                document.querySelector('.trusted-password-window__error')
                  ?.textContent ?? '',
            }),
            password,
          );
      ...
      assertJourney(
        !(await mainWindow.locator('.saved-passwords').innerText()).includes(
          password,
        ),
      ...
      await eventually(
        async () => {
          ...
          if (label !== 'Password masked')
            throw new Error(`password remains visible (${label})`);
        },
      ...
      await eventually(async () => {
        ...
        if (!copied)
          throw new Error(
            'clipboard did not receive the trusted-window export',
          );
      ...
      assertJourney(
        (
          await trustedWindow
            .locator('.trusted-password-window__body')
            .innerText()
        ).includes('Other apps and ordinary OkWork pages may read it.'),
      ...
      await passwordRow
        ...
        .click();
      await passwordRow
        .getByText('Delete this saved password?')
        .waitFor({ state: 'visible' });
      ...
      await mainWindow
        .getByRole('status')
        .getByText('No saved passwords yet')
        .waitFor({ state: 'visible' });
      ...
      if (app && clipboardCaptured) {
        try {
          await app.evaluate(
            ({ clipboard }, original) => clipboard.writeText(original),
            originalClipboard,
          );
          ...
          if (!restored)
            throw new Error('clipboard readback differs after restoration');
          console.log(
  ...
  async function main() {
    if (failures > 0) {
      throw new Error(
        `T-012 CONTRACT failed with ${failures} assertion${failures === 1 ? '' : 's'}`,
      );
    }
  ...
  void main().catch((error) => {
    console.error(
      `T-012: FAIL ${error instanceof Error ? error.message : String(error)}`,
    );
    process.exitCode = 1;
  });
}
