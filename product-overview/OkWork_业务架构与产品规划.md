...
|------|---|
| 文档状态 | ✅ 已确认 |
| 最近更新 | 2026-07-09（M5 远程规划轮：Q-002 模型 A / Q-003 远程机自管 已决） |
| 待决议题 | 0 项（见「规划议题追踪」） |

## 产品定位

OkWork 是以终端为主体的多工程、多并行会话工作台。它不绑定特定 CLI agent，而是在通用终端和 agent 管理器之间提供工程、会话、文件、git、通知和远程连续性能力。
...
1. 用户添加 Workspace，OkWork 记录工程根目录与显示名。
...
- [ ] **Host 独立可执行**：打包为纯 Node 单文件，可在远程机器独立运行；接入走 `ssh` 隧道 + WebSocket；远程机由 OkWork 自管（应用内配置 SSH 密钥或密码登录，密码凭据存系统钥匙串〔实现语义 safeStorage · BL-003 ADR-001〕，不做 ~/.ssh/config 导入）。
...
- [ ] **远程通知路径**：v1 仅做重连对账（UI 关闭期间收不到系统通知）；推送通道（手机 / 菜单栏常驻薄连接）留到以后。

#### 远程会话边界说明

- M1–M4 不交付远程会话，但架构按远程就绪设计，M5 兑现。
- 红利：分离架构顺手解锁「合盖离开 → 服务器上的 agent 继续跑 → 回来重连看徽标」的 tmux 式连续体验。
- 已知限制：UI 完全关闭期间收不到系统通知，靠重连对账兜底。
...
| Q-003 | 远程机配置来源 | ✅ 已决 | 不做 ~/.ssh/config 导入；远程机由 OkWork 自管（最近使用 + 手动添加），SSH 密钥或密码登录，密码凭据存系统钥匙串（实现语义 = safeStorage：加密密钥在钥匙串·密文落 userData·BL-003 ADR-001 · 2026-07-10 用户确认） | M5 详细范围 | 2026-07-09 |
