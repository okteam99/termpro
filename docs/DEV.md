...
- **☁ = 进入云端浏览器模式**(2026-08-26 用户拍板,此前只是"看一眼"):按下后工具栏
  **整体**驱动云端 —— 标签条/地址栏/前进后退/刷新全部经 `browser.*` RPC 落到远端
  (为此新增 `browser.reload` / `goBack` / `goForward`,历史尽头返回 `ok:false` 静默
  no-op);地址栏对空白页(about:blank / chrome://newtab)显示为空并自动聚焦,可直接
  输入导航;本机专属控件(出口选择/密码胶囊/↗系统浏览器)在云模式下让路;本机 webview
  标签保活语义不变,退出即恢复。云端标签状态靠 2s `listTabs` 轮询跟随(host 不推事件);
  🔴 用户钉选必须与轮询结果**对账**:远端(agent/页面自身)关掉被钉标签后要显式钉回
  活跃标签——只清成 null 时预览的 tabId prop 不变化,不会触发换流,画面会冻在死标签的
  最后一帧。起因是 2026-08-26 的白屏工单:☁ 只旁观时,空的云浏览器 = 纯白 canvas,与
  「坏了」不可区分;同一单还揪出预览建立后必须立即补发一次 `browser.resize`
  (ResizeObserver 首发早于推流建立,不补远端视口永远停在默认尺寸)。
