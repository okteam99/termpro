...
| `/sidebar/settings-about-entry` | `sidebar-settings-about` | Sidebar Account Entry (Login) | Workbench interaction preview | `OKWORK-F260826061325-Account-Menu-Settings-Panel`（增量 · 原 `OKWORK-F260613150158`） | 侧栏左下角账号入口：头像占位 + Login（不实现登录）→ 上弹菜单 Settings / About / Log out；Settings 打开左分类+右内容的全局 Settings 面板（General / Language / Browser / Passwords / Remote Hosts，禁止再套独立设置小卡）；About 仍为版本弹窗；Logout 未登录显示 Not signed in。入口为 footer 常驻，在所有工作台路由可见。 <!-- 🟡 2026-08-26: OKWORK-F260826061325-Account-Menu-Settings-Panel 本次变更：入口 Login + 账号菜单 + 全局两栏面板 --> |
...
- 2026-08-26 · `OKWORK-F260826061325-Account-Menu-Settings-Panel` · **不新增 route**。更新 `/sidebar/settings-about-entry`：入口文案 Login、账号菜单三项、全局两栏 Settings 面板承接原菜单里的设置项。`/settings/*` 内容预览路由保留。未改 token / 共享壳层基线，判定 L1。
