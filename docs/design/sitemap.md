...
| `/settings/browser-profiles` | `settings-browser-profiles` | Browser Settings & Profiles | Settings interaction preview | `OKWORK-F260807022801-Profile-Password-Vault`（BL-006） | 真实 520px Browser Settings 单列弹窗：链接/承载方式、Project 绑定 Profile、本机加密 Vault 数量与管理入口；覆盖 Profile 删除中、清理失败和加密不可用，常驻披露填入页面后的密码可被页面与 Agent 读取。Remote Host 权威位置与迁移留给 BL-007，Cookie 漫游留给 BL-008。 |
| `/settings/browser-passwords` | `settings-browser-passwords` | Saved Passwords | Settings interaction preview | `OKWORK-F260807022801-Profile-Password-Vault`（BL-006） | 本机 Vault 的脱敏密码管理：搜索、Profile 筛选、显示/复制/删除、空/加载/错误/加密不可用；普通 renderer 只见元数据，显式显示与复制进入隔离可信窗口，并披露 10 秒重遮罩及 60 秒条件清除剪贴板。BL-007 后续增量接入 Remote Host 权威位置。 |
| `/browser/password-save-fill` | `browser-password-save-fill` | Password Auto-save & Silent Fill | OkBrowser interaction preview | `OKWORK-F260807022801-Profile-Password-Vault`（BL-006） | 真实独立 OkBrowser 壳中的本机静默填充、登录成功后保存/更新、多账号确定选择、无匹配、Profile/exact-origin 隔离、结果不确定/登录失败不覆盖、加密不可用与普通 HTTP 停用；常驻提示页面与 Agent 可读取已填值。BL-007/BL-008 后续分别叠加远程权威与兼容 Cookie 漫游。 |
