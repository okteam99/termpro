...
- 2026-07-10 · `TERMPRO-F260710011342-Sidebar-Machine-Groups`（BL-004）· 新增 `/sidebar/machine-groups` 设计预览节点（Sidebar 机器分组主视图 + 组头连接生命周期 + 断线确定性回落 + 远程文件禁用提示）；`/workspace/add-workspace` 增量补齐远程目录浏览器的加载中/错误态（`fs.readdir` 权限拒绝 mock）。均为在既有 `/workspace/add-workspace` 全景上的增量细化，未推倒重画。
