...
| `/settings/browser-profiles` | `settings-browser-profiles` | Browser Settings & Profiles | Settings interaction preview | `OKWORK-F260807022801-Profile-Password-Vault`（BL-006）· `OKWORK-F260810051623-Remote-Profile-Authority`（BL-007 增量） | 真实 520px Browser Settings 单列弹窗：每个 Profile（含 Default）以普通文本显示本机或 Remote Host 存储位置；可选择 ready/兼容目标并经 Copy→Verify→Switch 迁移，覆盖迁移失败、远端离线、提交后原位置待清理与删除失败；Default 仍不可改名或删除，网络出口保持独立。Cookie 漫游留给 BL-008。 |
...
| `/browser/password-save-fill` | `browser-password-save-fill` | Password Auto-save & Silent Fill | OkBrowser interaction preview | `OKWORK-F260807022801-Profile-Password-Vault`（BL-006）· `OKWORK-F260810051623-Remote-Profile-Authority`（BL-007 增量） | 真实独立 OkBrowser 壳保留静默填充、保存/更新、多账号、Profile/exact-origin 隔离及失败保护；普通文本分开显示网络出口与密码存储位置。远端密码库离线时保存/填充 fail-closed 且不回退本机，页面 Cookie/session 可继续；兼容 Cookie 漫游仍留给 BL-008。 |
