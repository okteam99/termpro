...
| AC-5 | 目录展开自身，文件滚动/高亮，target=root 不高亮 | P0 | T-002, T-003, T-005, T-014, T-017, T-019 | ✅ |
...
| AC-8 | containment 使用一致显示路径表示，untrusted realpath fallback | P0 | T-007, T-015, T-020 | ✅ |
| AC-9 | 内部定位失败时走既有 fallback 且不改 File Panel | P0 | T-007, T-008, T-010, T-013, T-015, T-016, T-018, T-020 | ✅ |
| AC-10 | newer activation wins，stale expansion/highlight ignored；runtime locate view 不持久化重放 | P1 | T-009, T-012, T-017, T-019 | ✅ |
...
3. 读取阶段只构造本地 `pendingTopEntries`、`pendingCache` 和 `pendingExpanded`，不 dispatch mutation，也不 patch store mode。
...
2. 若 `effectiveRoot` 改变:
   ...
   - replace `expanded/cache/errPaths/statusMap/dirtyDirs` with the locate transaction result
...
3. 若 `effectiveRoot` 未改变:
   - merge `pendingCache`
   - replace expanded with the transaction's expanded set
...
| 15 | 用 `it.each` 拆分 path parsing、containment、highlight clear 子场景 | 🔵 Refactor | targeted tests | ☐ |
| 16 | 跑全量 `npm test`, `npm run typecheck`, `npm run lint` | 🔵 Refactor | exit 0 | ☐ |
...
REVIEW-ACK blueprint-claude-20260613T063211Z
