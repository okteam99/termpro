...
| locateScrollPath | string \| null | 否 | 必须等于可渲染 target row path | null | FilePanel.tsx row ref scrollIntoView 后清理或保留同 highlight |
...
6. The `inputs` reducer branch must explicitly guard the post-commit echo: if incoming `mode/rootPath/worktreePath` derive the same `effectiveRoot` and match the runtime inputs set by `locateCommit`, return without `applyRootChange`.
...
| 15 | 补缺 row TOCTOU、macOS case/Unicode、offscreen scroll、watcher cache merge、handler reject、non-current linked worktree 测试 | 🔴 Red | targeted tests | ☐ |
