...
tests:
  ...
  - id: T-011
    ...
    priority: P0
  - id: T-012
    file: src/renderer/state/__tests__/filePanelLocateRequest.test.ts
    function: locate_request_is_runtime_only_and_not_replayed_from_persistence
    covers_ac: ["AC-10"]
    level: unit
    priority: P1
  - id: T-013
...
| AC-9 | 内部定位失败时走既有 fallback 且不改 File Panel | P0 | T-007, T-008, T-010, T-013 | ✅ |
| AC-10 | newer activation wins，stale expansion/highlight ignored；runtime request 不持久化重放 | P1 | T-009, T-012 | ✅ |
...
Then File Panel returns a fallback result
...
### Scenario: T-012 locate request is runtime-only
**优先级**: P1
**类型**: 回归
**测试层级**: unit

```gherkin
Given tab "t1" has a pending filePanelLocateRequest
When the app state is converted to PersistedState
Then PersistedTab for "t1" does not contain filePanelLocateRequest
When the persisted state is hydrated
Then no locate request is replayed
```
...
Then no File Panel locate request is enqueued
 And the active tab File Panel state is unchanged
 And existing fallback handling is invoked for the target path
```
...
| 字段 | 类型 | 必填 | 校验规则 | 默认值 | 备注 |
|------|------|------|----------|--------|------|
| activeLocateRequestId | number \| null | 否 | 等于当前处理中 request.id | null | controller 内 async stale gate |
...
4. Controller 收到 request 后构建全部匹配 candidate roots:
   - effective WorkTree root, if target is inside it
   - effective Root path, if target is inside it
5. Candidate 排序规则: 路径更深、更具体的 root 优先；嵌套 WorkTree 与 enclosing Root 同时命中时 WorkTree 胜出；只有没有更具体候选时才保留 current mode。
6. 如果 chosen root 的 mode 与当前 mode 不同，只 patch `mode`，不写 `rootPath/worktreePath` binding。
7. Candidate containment 使用 normalized displayed path 的 separator-aware comparison。`fs.realpath` 仅用于 revalidation/trust check；如果 realpath 返回 null，或无法映射回 displayed root chain，返回 false。
...
4. 任一级 `readdir` 失败、缺 row、realpath 不可信或 request id stale，返回 false，现有 mode/bindings/expanded/cache/highlight 完全不变。
...
Store 分配的 `request.id` 是唯一 stale token。Controller 在 `locateTarget` 开始时把 `activeLocateRequestId` 设为该 id；所有 async continuation 在写 state 前检查 `activeLocateRequestId === request.id`。新 request、refresh、tab switch、user toggle 均清理旧 highlight，并使旧 request id stale。
...
| T-012 | `locate_request_is_runtime_only_and_not_replayed_from_persistence` | ☐ |
...
| 3 | 写 store runtime locate request persistence tests | 🔴 Red | `npm test -- src/renderer/state/__tests__/filePanelLocateRequest.test.ts` | ☐ |
...
| 10 | 实现 transactional ancestor load + fallback result + request.id stale gate | 🟢 Green | locateTarget tests | ☐ |
| 11 | 写 inactive-tab routing fallback test | 🔴 Red | terminal routing tests | ☐ |
| 12 | 实现 active tab guard and existing fallback path | 🟢 Green | terminal routing tests | ☐ |
...
| stale locate request 改写新状态 | 使用 store `request.id` 作为 controller 唯一 stale token |
...
REVIEW-ACK blueprint-claude-20260613T061517Z
