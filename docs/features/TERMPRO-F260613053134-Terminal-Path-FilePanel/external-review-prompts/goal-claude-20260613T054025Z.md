...
acceptance_criteria:
  - id: AC-1
    description: "Given a terminal fs link resolves to an existing file or directory, when the user activates it, then TermPro attempts File Panel handling for the owning tab before external/viewer fallback."
    category: functional
    priority: P0
    test_refs: []
    ui_refs: []
  - id: AC-2
    description: "Given the owning tab File Panel is in WorkTree mode and the target path is inside that tab's effective WorkTree root, when the link is activated, then the File Panel remains in WorkTree mode, expands the ancestor chain, and scrolls/selects the target row."
    category: functional
    priority: P0
    test_refs: []
    ui_refs: []
  - id: AC-3
    description: "Given the owning tab File Panel is in Root mode and the target path is inside that tab's effective Root path, when the link is activated, then the File Panel remains in Root mode, expands the ancestor chain, and scrolls/selects the target row."
    category: functional
    priority: P0
    test_refs: []
    ui_refs: []
  - id: AC-4
    description: "Given the current File Panel mode cannot contain the target but the target is inside the owning tab's effective WorkTree root or Root path, when the link is activated, then TermPro switches to the first matching mode in priority order WorkTree then Root, preserving the tab's existing root/worktree bindings and adding the target expansion state."
    category: functional
    priority: P0
    test_refs: []
    ui_refs: []
  - id: AC-5
    description: "Given an internal File Panel handling target is a directory, when it is activated, then its ancestor chain and the directory itself are expanded; given the target is a file, then its parent chain is expanded and the file row is scrolled into view and selected."
    category: functional
    priority: P0
    test_refs: []
    ui_refs: []
  - id: AC-6
    ...
    ui_refs: []
  - id: AC-7
    description: "Given existing terminal link parsing supports file://, absolute, home, relative, and :line:col forms, when this feature is delivered, then those supported forms keep resolving correctly; internally handled :line:col links use the stripped file path for File Panel location."
    category: functional
    priority: P1
    test_refs: []
    ui_refs: []
  - id: AC-8
    description: "Given a target path is compared to Root or WorkTree containment, when TermPro decides whether it is inside a root, then it uses normalized, separator-aware path comparison after existing path decoding/line-col stripping, with macOS case-insensitive behavior and symlink-safe real paths where available."
    category: functional
    priority: P0
    test_refs: []
    ui_refs: []
  - id: AC-9
    description: "Given internal File Panel handling cannot complete because the path is outside both roots, disappears, or the tree cannot be read, when the link is activated, then TermPro uses the existing external/viewer fallback instead of silently doing nothing."
    category: functional
    priority: P0
    test_refs: []
    ui_refs: []
...
优先级解释：目标面板永远是触发该链接的 Terminal 所属 Tab 的 File Panel。若该 File Panel 当前选中的上下文可以容纳目标路径，则尊重当前上下文；例如 Root 模式下的 root 内路径在 Root 中定位，WorkTree 模式下的当前 worktree 内路径在 WorkTree 中定位。若当前上下文不能容纳目标路径，再按 WorkTree → Root → 外部/viewer 处理兜底。

术语解释：effective WorkTree root 指该 Tab File Panel 已绑定的 `worktreePath`；若未绑定，则指 File Panel 当前可推导的 `autoWorktree`。effective Root path 指该 Tab File Panel 已绑定的 `rootPath`；若未绑定，则指 File Panel 当前可推导的 `autoRoot`。

定位语义：内部处理是 location-only，不自动打开文件内容。目录目标展开祖先链并展开目录本身；文件目标展开父目录链、滚动到文件行并选中/高亮。若用户要打开内容，沿用 File Panel 中点击文件行的既有行为。
...
| 变化 | 验证方式 |
|------|----------|
| 点击属于当前 WorkTree 的终端文件路径，会在 owning tab 的 File Panel WorkTree 视图中展开并定位 | 在 WorkTree 模式输出/点击 `src/renderer/App.tsx` 或 worktree 内绝对路径 |
| 点击属于当前 Root 的终端文件路径，会在 owning tab 的 File Panel Root 视图中展开并定位 | 在 Root 模式输出/点击 root 内绝对路径 |
...
| ID | 描述 | 优先级 | 覆盖测试 |
|----|------|--------|----------|
| AC-1 | Given a terminal fs link resolves to an existing file or directory, when the user activates it, then TermPro attempts File Panel handling for the owning tab before external/viewer fallback. | P0 | |
| AC-2 | Given the owning tab File Panel is in WorkTree mode and the target path is inside that tab's effective WorkTree root, when the link is activated, then the File Panel remains in WorkTree mode, expands the ancestor chain, and scrolls/selects the target row. | P0 | |
| AC-3 | Given the owning tab File Panel is in Root mode and the target path is inside that tab's effective Root path, when the link is activated, then the File Panel remains in Root mode, expands the ancestor chain, and scrolls/selects the target row. | P0 | |
| AC-4 | Given the current File Panel mode cannot contain the target but the target is inside the owning tab's effective WorkTree root or Root path, when the link is activated, then TermPro switches to the first matching mode in priority order WorkTree then Root, preserving the tab's existing root/worktree bindings and adding the target expansion state. | P0 | |
| AC-5 | Given an internal File Panel handling target is a directory, when it is activated, then its ancestor chain and the directory itself are expanded; given the target is a file, then its parent chain is expanded and the file row is scrolled into view and selected. | P0 | |
...
| AC-7 | Given existing terminal link parsing supports file://, absolute, home, relative, and :line:col forms, when this feature is delivered, then those supported forms keep resolving correctly; internally handled :line:col links use the stripped file path for File Panel location. | P1 | |
| AC-8 | Given a target path is compared to Root or WorkTree containment, when TermPro decides whether it is inside a root, then it uses normalized, separator-aware path comparison after existing path decoding/line-col stripping, with macOS case-insensitive behavior and symlink-safe real paths where available. | P0 | |
| AC-9 | Given internal File Panel handling cannot complete because the path is outside both roots, disappears, or the tree cannot be read, when the link is activated, then TermPro uses the existing external/viewer fallback instead of silently doing nothing. | P0 | |
...
REVIEW-ACK goal-claude-20260613T054025Z
