...
- **AI drives the browser (built-in MCP)** — a session's agent gets two explicit tool families on a per-terminal MCP endpoint: `inner_browser_*` (OkWork's built-in logged-in webview) and `headless_remote_browser_*` (Chromium on the remote machine). They are not auto-routed. Wire it with `claude mcp add --transport stdio okbrowser -- okwork-browser-mcp` (do not snapshot `$OKWORK_BROWSER_MCP_URL` — it embeds a terminal id). See `docs/DEV.md §4.8`, `§4.13`.
...
- **Cloud browser** (August 2026) — on a remote machine with Chromium installed, `headless_remote_browser_*` MCP tools drive a headless Chromium over CDP on that machine. It is headless by default (zero picture traffic); pressing ☁ in the browser panel switches the whole panel into **cloud-browser mode**. Frames travel on their own WebSocket. `inner_browser_*` always stays on the laptop webview. See `docs/DEV.md §4.13`.
