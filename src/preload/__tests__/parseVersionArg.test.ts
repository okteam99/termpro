...
describe('parseVersionArg', () => {
  it('parseVersionArg_extracts_version_from_arg', () => {
    expect(parseVersionArg(['--okwork-version=0.3.12'])).toBe('0.3.12');
    // Extra args around it should not matter
    expect(parseVersionArg(['--foo', '--okwork-version=1.2.3', '--bar'])).toBe('1.2.3');
    // Takes the FIRST match
    expect(
      parseVersionArg(['--okwork-version=0.1.0', '--okwork-version=0.2.0']),
    ).toBe('0.1.0');
  });

  it('parseVersionArg_returns_empty_on_all_failure_modes', () => {
    // value is empty string after `=`
    expect(parseVersionArg(['--okwork-version='])).toBe('');
    // arg has no `=` (no prefix match with `--okwork-version=`)
    expect(parseVersionArg(['--okwork-version'])).toBe('');
    // completely absent
