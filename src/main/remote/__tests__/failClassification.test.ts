...
import { failReasonCopyMap, type FailReason } from '../../../shared/remoteHost';
...
describe('AC-2 failure taxonomy single source', () => {
  it('T-004 failReasonCopyMap 覆盖全部 9 类,每类都有 label', () => {
    const copy = failReasonCopyMap();
    for (const reason of ALL_REASONS) {
      expect(copy[reason]).toBeDefined();
      expect(typeof copy[reason].label).toBe('string');
      expect(copy[reason].label.length).toBeGreaterThan(0);
    }
    expect(Object.keys(copy).sort()).toEqual([...ALL_REASONS].sort());
  });
