...
class Guest implements PasswordGuestSender {
  ...
  constructor(readonly id: number, private readonly url: string) {}
...
function vault(): PasswordVaultPort {
  const entry: DecryptedPasswordCredentialLike = {
    id: 'entry-1', profileId: PROFILE_A, origin: 'https://accounts.example.test', username: 'alice',
    password: SECRET, createdAt: 1, updatedAt: 1, lastUsedAt: 1,
  };
  return {
    isAvailable: () => true,
    listMetadata: () => [{ id: entry.id, profileId: entry.profileId, origin: entry.origin, username: entry.username, createdAt: 1, updatedAt: 1, lastUsedAt: 1 }],
    lookup: (profileId, origin) => profileId === entry.profileId && origin === entry.origin ? [entry] : [],
    getDecrypted: (id) => {
      if (id !== entry.id) throw Object.assign(new Error('not found'), { code: 'VAULT_ENTRY_NOT_FOUND' });
      return entry;
    },
    upsert: vi.fn(), deleteEntry: vi.fn(() => false), deleteProfile: vi.fn(() => false),
  };
}

describe('browser password trust boundaries', () => {
  it('test_AC8_rejects_untrusted_vault_access_while_disclosing_dom_and_clipboard_exports', () => {
    const controller = new PasswordVaultController({ vault: vault(), isProfileActive: () => true, onMetadataChanged: vi.fn() });
    const websiteOrAgent = new Guest(100, 'https://accounts.example.test/login');
    const wrongProfileGuest = new Guest(101, 'https://accounts.example.test/login');
    ...
    expect(controller.lookup(websiteOrAgent.id, {})).toEqual({ kind: 'unavailable' });
    expect(controller.acceptCandidate(websiteOrAgent.id, { nonce: 'n', username: 'alice', password: SECRET })).toBe(false);
    expect(controller.fillEntry(websiteOrAgent.id, 'entry-1')).toBe(false);
    // A registered guest in another Profile does not receive credentials from Profile A.
    expect(controller.lookup(wrongProfileGuest.id, {})).toEqual({ kind: 'none' });
